...
colmap = matplotlib.colormaps['Set1']

# Scaling
def get_scaledvar_snapshot(var, it, key, scaletype='default', slope=None):
  '''
  ...
  def get_normunits(xnormstr, rhonormsub):
    ...
    return {key:CGS2norm[value] for (key,value) in units_CGS.items() if value}
  ...
  if scaletype == 'code':
    ...
    if env.mode == 'shells':
      ...
      r_scale, rho_scale = env.R_0, getattr(env, env.rhoNorm)
    elif env.mode == 'PWN':
  ...
  x /= r_scale
  z *= get_varscaling(var, rho_scale)
  xlabel = var_exp['x'] + units['x']
  zlabel = var_exp[var] + units[var]
  if slope:
    logx = np.log10(x)
...
def plot_multi(varlist, it, key='Last', scaletype=None):
  ...
  for var, k, ax in zip(varlist, range(Nk), axes):
    title = plot1D(var, False, it, key, scaletype, ax)#, col=colors[k])
...
def plot_comparison(varlist, it, key='Last', scaletype=None, col='Zone'):
  ...
  for var in varlist:
    ...
    title = plot1D(var, False, it, key, scaletype, ax, col=col, label=varlabel)
  
  plt.suptitle(title)
  plt.legend()
  plt.tight_layout()

def plot1D(var, slope, it, key, scaletype=None, ax=None, code_units=False, line=True, col='Zone', **kwargs):
  ...
  if col == 'Zone' or var == 'zone':
    ...
    k = len(ax.lines)
    cols = np.array([cm.Set1([range(k)])[0,-1] for i in range(len(r))])
    scatter = ax.scatter(r, z, c=cols, lw=1, zorder=2, **kwargs)
