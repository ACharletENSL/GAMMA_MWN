...
def update_simFile(filepath, env):
  gridvars = {
    'rmin0':env.rmin0, 'rmax0':env.rmax0, 'Ncells':env.Ncells
  }
  physvars = {}
  if env.mode == 'MWN':
    physvars = {
      ...
      't_sd':env.t_0, 't_start':env.t_start, 'm_fade':m
    }
    ...
    for line in inFile:
      if line.startswith('static double'):
        ...
        if var in vars2update:
          ...
          elif var == 'rmin0' and env.mode == 'shells':
            line = line.replace(l[4], 'R_0-1.1*D04')
          elif var == 'rmax0' and env.mode == 'shells':
            line = line.replace(l[4], 'R_0+1.1*D01')
          elif var == 'm_fade':
