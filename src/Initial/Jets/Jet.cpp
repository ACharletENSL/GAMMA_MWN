...
#define PROFILE_ TOPHAT_          // jet profile.
// available choices: TOPHAT_, POWLAW_, GAUSS_, HOLLOW_

bool onedim = false;

// set jet and CBM parameters
...
// intermediate quantities (Shock description)
  ...
  static double tstart = pow(Eiso*(17.)/(8.*PI*n_ext* mp_*pow(c_,5.)*lfacShock2), 1./3.);
#else
  static double tstart = tinit;
  static double lfacShock2 = Eiso * (17.- 4.*k)/(8.*PI*n_ext*mp_*pow(tstart,3)*pow(c_,5));
...
static double calcProfile(double th){
  // returns profile function of the jet
  // only a "smooth" tophat for now
  return 1.0/cosh(pow(th/th_c0, b));
}

static void calcJet(double r, double th, double t, double *rho, double *u, double *p){
  // returns normalised primitive variables for BM solution at position r (denormalised).
  // each theta track is set as part of BM solution of energy Eiso(theta)
  double E = Eiso*calcProfile(th);

  // repeating from above (these are local)
  double lfacShock2 = E * (17.- 4.*k)/(8.*PI*n_ext*mp_*pow(t,3)*pow(c_,5));
    // Blandford&McKee(1976) eq. 69
...
void loadParams(s_par *par){
  ...
  par->nmax      = 500;               // max number of cells in MV direction
